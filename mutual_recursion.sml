(* an example of mutual recurson: a little "state machine" for deciding if a list of ints alternates between 1 and 2, not ending with a 1.

 ( There are simpler solutions to this problem, but this approach works for
...
fun no_zeros_or_empty_string_t2_alternate x = 
    case x of 
	Baz s => size s > 0 
     | Quxx y  => no_zeros_empty_strings_t1_alternate(
